...
ユーザーが今日の習慣を確認・達成し、日々の積み上げを Activity Log で振り返り、その進捗を画像として共有し、必要に応じて Web Push リマインダーを設定できる主要画面。
...
- `HomeMainContent`: Activity Log、共有、通知設定、今日の習慣を構成
- `ActivityLog`: 365日グリッド
- `ShareButton`: 共有カード生成済みファイルを Web Share API へ渡す
- `NotificationSettingsCard`: 通知権限・端末Subscription・通知時刻を管理する
- `TodaySummary`: 今日の完了数
- `HabitList`: 習慣カード一覧
- `AddHabitDialog`: 習慣追加
...
Activity Log の下に `NotificationSettingsCard` を表示する。
...
- JST の通知時刻 `HH:MM`
- 現在の端末が未登録の場合の端末登録導線
- 通知拒否・非対応ブラウザ・VAPID未設定時の状態表示
- iPhone / iPad では Home Screen Web App から利用する案内

通知権限はページ表示時には要求しない。ユーザーが通知をONにする、または端末登録操作を行ったタイミングで `Notification.requestPermission()` を呼ぶ。

通知をONにする際は、概ね次の順に処理する。

1. 通知権限を確認・要求
2. Service Worker の Push Subscription を取得または作成
3. `PUT /api/notifications` で現在端末を登録
4. `PATCH /api/notifications` で通知設定を有効化

通知OFF時は、まずサーバー設定をOFFにして送信対象から外し、その後現在端末の Subscription cleanup を行う。

アカウント側の通知がすでにONでも、現在ブラウザの Subscription が未登録なら「この端末でも通知を受け取る」導線を表示する。

通知本文・Subscription管理UIに習慣名を含めない。
...
| 通知設定取得 | `GET /api/notifications` | リマインダー状態表示 |
| 通知設定更新 | `PATCH /api/notifications` | ON/OFF・時刻更新 |
| 端末登録 | `PUT /api/notifications` | Subscription保存 |
| 端末解除 | `DELETE /api/notifications` | Subscription削除 |
...
- 操作ターゲットを小さくしすぎない
- Push通知がブラウザ / OS の制約を受ける場合は利用条件を明示する
...
- Push通知拒否 / 非対応 / runtime未設定: リマインダーカード内で理由を表示する
