...
2. 各対象習慣について、`daily_record` テーブルから直近の達成日（最大 `date`）を取得する。
3. 以下のいずれかに該当する習慣を「ストリーク切れ」と判定する。ただし `habit.currentStreak` がすでに `0` の場合はスキップ。
...
`spec.md` の「その日 24:00 時点の対象習慣数」に準拠し、日付 D の対象習慣を以下のように判定する。
