...
- 期限は全習慣共通で **翌日 00:00 JST**（当日 24:00 と同義）のため、習慣ごとの `deadTime` カラムは持たない
...
- `habit.deadTime`: 現行では必須だが、MVP では全習慣で締め時刻を **翌日 00:00 JST**（当日 24:00 と同義）として扱うため、個別期限としては使わない
- `habit.isArchived`: 現行では boolean 管理だが、MVP 目標設計では過去日の分母固定に対応するため `archivedAt` で扱う
- `habit.emoji`: 現行では `NOT NULL` だが、MVP の API では未設定を `null` として扱う。スキーマ移行までは DB 上の空文字を未設定として保持し、API 入出力では未設定として正規化する前提とする
...
- `emoji`: 習慣カードに表示する任意のアイコン。MVP では未設定を許可し、API では未設定時に `null` を返す
...
- 未達成日はレコードを作成しない
- `date` は JST（Asia/Tokyo）基準で確定した達成日
...
`user.timezone` カラムは現行 Better Auth 拡張の保持項目として残るが、MVP の日付切り替え、締め時刻判定、`daily_record.date` の解釈には使用しない。これらはすべて JST（Asia/Tokyo）固定で扱う。
...
- 判定境界は各日付が終わる翌日 `00:00 JST` とし、`archivedAt = D+1日 00:00 JST` ちょうどのアーカイブは、その日が終わるまでは対象だったものとして日付 D の分母に含める
- これにより、後日アーカイブされた習慣があっても、過去日の Activity Log の色を同じ条件で再計算できる

実装上は、JST で見た各日付が終わる翌日 `00:00 JST` 時点を基準に対象習慣数を求める想定とする。
